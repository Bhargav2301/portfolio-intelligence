...
class RunCoordinator:
    ...
    def create(self, owner_email: str, request: AnalysisRunRequest) -> AnalysisRun:
        ...
        level = EventLevel.WARNING if status == RunStatus.BLOCKED else EventLevel.INFO
        message = (
            "Readiness checks blocked this run"
            if status == RunStatus.BLOCKED
            else "Run accepted"
        )
        self._event(run, level, "policy", message)
    ...
    async def execute(self, run_id: str, request: AnalysisRunRequest) -> None:
        ...
        self._event(
            ...
            EventLevel.INFO,
            "orchestrator",
            "Portfolio snapshot locked for analysis",
        )
        ...
        try:
            ...
            self._event(
                ...
                EventLevel.INFO,
                "langgraph",
                "PI portfolio workflow completed",
            )
