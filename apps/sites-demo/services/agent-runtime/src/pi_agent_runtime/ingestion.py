...
def _derive_symbol(source_name: str) -> str:
    symbol = re.sub(r"[^A-Z0-9]+", "", source_name.upper())[:24]
    if not symbol:
        raise NormalizationError("instrument symbol could not be derived")
    return symbol


def normalize_consolidated_rows(
    frame: pd.DataFrame,
    *,
    filename: str,
    source_hash: str,
    portfolio_name: str = "D1 Portfolio",
    instrument_mappings: dict[str, tuple[str, str, str]] | None = None,
) -> dict[str, Any]:
    ...
    unresolved_mappings: set[str] = set()
    confirmed_mappings = instrument_mappings or {}
    ...
    for index, row in frame.iterrows():
        ...
        alias = confirmed_mappings.get(source_name)
        if alias:
            symbol, canonical_name, analysis_symbol = alias
        else:
            symbol = _derive_symbol(source_name)
            canonical_name = source_name
            analysis_symbol = None
            unresolved_mappings.add(source_name)
        expected_suffix = ".NS" if exchange == "NSE" else ".BO" if exchange == "BSE" else ""
        if alias and (not expected_suffix or not analysis_symbol.endswith(expected_suffix)):
            raise NormalizationError(
    ...
    if unresolved_mappings:
        warnings.append(
            f"{len(unresolved_mappings)} instrument mapping(s) were derived from names; "
            "confirm exchange tickers before publication"
        )
    ...
    public_lots = [
        {
            ...
            if key not in {"source_investment", "source_latest_value", "current_price"}
        }
        for lot in lots
    ]
    return {
...
def normalize_consolidated_workbook(
    path: Path, *, portfolio_name: str = "D1 Portfolio"
) -> dict[str, Any]:
    content = path.read_bytes()
    workbook = pd.ExcelFile(path)
    sheet_name = "stocks" if "stocks" in workbook.sheet_names else workbook.sheet_names[0]
    frame = pd.read_excel(workbook, sheet_name=sheet_name)
    return normalize_consolidated_rows(
        frame,
        filename=path.name,
...
def main() -> None:
    ...
    result = normalize_consolidated_workbook(
