...
import type { ChatResponse, DashboardData, Evidence, Position } from "../lib/types";

type View = "overview" | "research" | "activity" | "scenario";
      ...
      <main className="workspace">
        <header className="topbar">
          <div>
            ...
            <h1>{view === "overview" ? data.portfolio.name : titleForView(view)}</h1>
          </div>
          <div className="topbar-actions">
            <div className="data-state"><span /> Demo data · {formatDate(data.asOf)}</div>
            <button className="primary-button" onClick={() => setTransactionOpen(true)}>Add transaction</button>
          </div>
        </header>

        {error && <div className="error-banner" role="alert">{error}</div>}
        {view === "overview" && <Overview data={data} onViewChange={setView} />}
        {view === "research" && <ResearchView data={data} />}
        {view === "activity" && <ActivityView data={data} onData={setData} onError={setError} />}
        {view === "scenario" && <ScenarioView data={data} />}
      </main>

      <ResearchCopilot data={data} />
      {transactionOpen && (
        <TransactionDialog
          data={data}
    ...
    <div className="view-stack">
      <section className="metric-grid" aria-label="Portfolio summary">
        <article className="metric-card hero-metric">
          <div className="metric-heading"><span>Total portfolio value</span><span className="demo-pill">DEMO</span></div>
        ...
        <tbody>
          {positions.map((position) => (
            <tr key={position.symbol}>
              ...
              <td><strong>{money.format(position.currentPrice)}</strong><small className="source-note">Demo snapshot</small></td>
...
function ResearchView({ data }: { data: DashboardData }) {
  return (
    <div className="view-stack">
      ...
      <div className="disclosure-card"><strong>Demo evidence only</strong><p>These fictional source records demonstrate the evidence-gating workflow. They are not live filings, news, research, or an invitation to invest.</p></div>
    </div>
  );
}
...
function ResearchCopilot({ data }: { data: DashboardData }) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "assistant", text: `I can explain ${data.portfolio.name} using deterministic analytics and its attached demo evidence. Try asking about performance, concentration risk, sources, or scenarios.` },
  ]);
  ...
  return (
    <aside className="copilot">
      ...
      <p className="copilot-disclaimer">Demo research intelligence. Not investment advice.</p>
    </aside>
  );
}
...
function titleForView(view: View) { return view === "research" ? "Evidence & research" : view === "activity" ? "Transaction history" : "Scenario lab"; }
