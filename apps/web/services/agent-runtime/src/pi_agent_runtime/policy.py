...
from .models import AnalysisRunRequest, HoldingSnapshot, PolicyCheck, SymbolResult, utc_now
...
def readiness_checks(request: AnalysisRunRequest) -> list[PolicyCheck]:
    ...
    for symbol in selected:
        ...
        if not holding.analysis_symbol:
            checks.append(PolicyCheck(
                ...
                severity="block",
                symbol=symbol,
                message=f"{symbol} does not have a confirmed TradingAgents market-data symbol.",
            ))
        ...
        if age_minutes > request.policy.data_max_age_minutes:
            checks.append(PolicyCheck(
                ...
                severity="block",
                symbol=symbol,
                message=f"{symbol} price is {age_minutes:.0f} minutes old; refresh before analysis.",
            ))
    ...
    if not 99 <= total_allocation <= 101:
        checks.append(PolicyCheck(
            ...
            severity="block",
            message=f"Portfolio allocation totals {total_allocation:.2f}%, outside the 99–101% reconciliation band.",
        ))
    ...
    checks.append(PolicyCheck(
        ...
        severity="pass",
        message="Analysis can produce a proposal, but every order requires explicit human confirmation.",
    ))
    return checks


def result_policy_checks(request: AnalysisRunRequest, holding: HoldingSnapshot, result: SymbolResult) -> list[PolicyCheck]:
    checks: list[PolicyCheck] = []
    if holding.allocation_percent >= request.policy.max_position_weight_percent and result.trader_action == "Buy":
        checks.append(PolicyCheck(
            code="position_cap",
            severity="block",
            symbol=holding.symbol,
            message=(
                f"Buy proposal blocked: {holding.symbol} is already {holding.allocation_percent:.2f}% "
                f"versus the {request.policy.max_position_weight_percent:.2f}% policy cap."
            ),
        ))
    else:
        checks.append(PolicyCheck(
